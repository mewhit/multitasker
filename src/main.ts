...
function findMatchingVsCodeTerminal(
  session: VsCodeSessionRequest,
  windowEntry: VsCodeWindowEntry
): VsCodeTerminalRegistration | null {
  ...
  if (sessionTerminalPid !== undefined) {
    ...
    if (exactPidMatch) return exactPidMatch;
  }
...
function createRecurringTask(textValue: unknown, timeValue: unknown, daysValue: unknown): RecurringTaskState | null {
  const text = typeof textValue === 'string' ? textValue.trim() : '';
...
function isRecurringTaskDue(task: RecurringTaskState, now: Date): boolean {
  ...
  if (!task.daysOfWeek.includes(now.getDay())) return false;
...
function startTerminalUpdateServer(): void {
...
function setupIpc(): void {
  ...
  ipcMain.handle('session:remove', (_e, id: string) => {
  ...
  ipcMain.handle('recurring-task:add', (_event, text: unknown, time: unknown, daysOfWeek: unknown) =>
    createRecurringTask(text, time, daysOfWeek)
  );
